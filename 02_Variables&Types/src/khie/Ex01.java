 ...
 * - 필요한 데이터를 저장할 곳을 미리 선언해 두고, 나중에 계속해서 사용하기 위한 변수를 만듦
 * - 데이터를 저장하기 위해 생성되는 메모리 공간은 목적에 따라 크기와 특징이 다른데, 
 * 	 이를 자료형이라고 함. 즉, 메모리 공간의 자료형에 따라 저장할 수 있는 값의 종류와 크기가 결정됨
 * 
 * 변수를 선언하는 방법
 * - 형식) 자료형 변수명;
 * 
 ...
 * 
 * 1 byte = 8 bit(1 bit는 정보의 최소단위, 0/1 둘 중 하나가 들어갈 수 있으며 이들의 조합으로 이루어짐)
 * 맨 앞의 한 bit을 양수/0/음수 구분을 위해 부호 bit으로 사용
 * (-)------- => -27~27-1 = -128~127 까지가 표현 범위임
 * 
 * 
 * 2. 실수형(실수자료형) : float(4바이트), double(8바이트-기본자료형)
 * 3. 단일문자형 : char(2바이트~3바이트)
 * 4. 논리형(논리자료형) : boolean(true/false) 1바이트
 * 
...
public class Ex01 {
	...
	public static void main(String[] args) {
		...
		// 2. 변수 초기화 - 변수에 값을 저장한다는 의미(= 기호 사용)
		su = 100; // 왼쪽 : 변수명 LV
...
//		su += 35;
		...
		System.out.println(su); // 변수에 저장된 값을 출력해주세요.
